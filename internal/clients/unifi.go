...
import (
	...
	clusterv1beta1 "github.com/devantler-tech/provider-upjet-unifi/apis/cluster/v1beta1"
	namespacedv1beta1 "github.com/devantler-tech/provider-upjet-unifi/apis/namespaced/v1beta1"
)
...
const (
	...
	errUnmarshalCredentials = "cannot unmarshal unifi credentials as JSON"
)

// Keys recognised in the ProviderConfig credentials JSON. They map verbatim to
// the ubiquiti-community/unifi Terraform provider configuration arguments.
const (
	unifiKeyAPIURL         = "api_url"
	unifiKeyUsername       = "username"
	unifiKeyPassword       = "password"
	unifiKeyAPIKey         = "api_key"
	unifiKeySite           = "site"
	unifiKeyAllowInsecure  = "allow_insecure"
	unifiKeyHardwareID     = "hardware_id"
	unifiKeyCloudConnector = "cloud_connector"
)
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		// These keys map to the ubiquiti-community/unifi Terraform provider's
		// configuration arguments. Only keys present in the credentials JSON
		// are forwarded, so callers can mix and match (e.g. api_key OR
		// username/password).
		...
		for _, key := range []string{
			...
			unifiKeyAPIKey,
			unifiKeySite,
			unifiKeyAllowInsecure,
			unifiKeyHardwareID,
			unifiKeyCloudConnector,
		} {
