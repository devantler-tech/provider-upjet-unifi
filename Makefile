...
PROJECT_NAME ?= provider-upjet-unifi
PROJECT_REPO ?= github.com/devantler-tech/$(PROJECT_NAME)
...
XPKG_REG_ORGS ?= ghcr.io/devantler-tech
# NOTE(hasheddan): skip promoting on xpkg.crossplane.io as channel tags are
# inferred.
XPKG_REG_ORGS_NO_PROMOTE ?= ghcr.io/devantler-tech
XPKGS = $(PROJECT_NAME)
...
xpkg.build.provider-upjet-unifi: do.build.images
...
schema-version-diff:
	...
	@if [ ! -f config/generated.lst ]; then \
		echo "config/generated.lst not present (run 'make generate'); skipping native schema version diff"; \
		exit 0; \
	fi; \
	if ! git cat-file -e "$${GITHUB_BASE_REF}:config/schema.json" 2>/dev/null; then \
		echo "No base config/schema.json on $${GITHUB_BASE_REF} to diff against; skipping native schema version diff"; \
		exit 0; \
	fi; \
...
go-floor.test:
	@$(INFO) checking the advisory-free Go toolchain floor
